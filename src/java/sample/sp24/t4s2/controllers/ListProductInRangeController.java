...
@WebServlet(name = "ListProductInRangeController", urlPatterns = {"/ListProductInRangeController"})
public class ListProductInRangeController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            list = dao.getProductInRange(priceStart, priceEnd);
            List<String> listCategory = dao.getCategory();
            if (listCategory != null) {
                if (listCategory.size() > 0) {
                    request.setAttribute("LIST_IN_RANGE", listCategory);
                }
            }
            if (list != null && list.size() > 0) {
                request.setAttribute("LIST_IN_RANGE", list);
                url = SUCCESS;
            }
        } catch (Exception e) {

...
@WebServlet(name = "ReportRevenueByMonthController", urlPatterns = {"/ReportRevenueByMonthController"})
public class ReportRevenueByMonthController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String url = ERROR;
        try {
            String inputDate = request.getParameter("search");
            int index = inputDate.indexOf("-");
            String year = inputDate.substring(0, index);
            String month = inputDate.substring(index+1);
            RevenueDAO revenueDAO = new RevenueDAO();
            double revenue = revenueDAO.getRevenueByMonth(year,month);
            if (revenue >= 0) {
                request.setAttribute("REVENUEBYMONTH", revenue);
                url = SUCCESS;
            }
        } catch (Exception e) {
